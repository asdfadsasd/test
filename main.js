...
global.translate = {};
// cache the language files
function loadLanguages() {
  const files = readdirSync(join(__dirname, './src/languages'));
  for (const file of files) {
    const lang = file.replace('.json', '');
    global.translate[lang] = JSON.parse(readFileSync(join(__dirname, './src/languages', file), 'utf8'));
  }
}
loadLanguages();
